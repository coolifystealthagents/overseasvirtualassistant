...
import { september10BlogPosts } from "../september-10-blog";
...
const september3Paths = Object.keys(september3Meta).map(
  (slug) => `/blog/${slug}`,
);
const aug23Paths = Object.entries(aug23Meta).map(
  ([slug, value]) => `/${(value as { family: string }).family}/${slug}`,
);
const aug21Paths = Object.entries(aug21Meta).map(
  ([slug, value]) => `/${(value as { family: string }).family}/${slug}`,
);
...
export async function GET() {
  const pages = [
    ...
    "/services",
    "/blog",
    "/blog/top-30-overseas-virtual-assistant-companies",
    "/research",
    "/alternatives",
    "/contact",
    "/privacy",
  ...
  const urls = [
    ...
    ...september14BlogPosts.map((p) => "/blog/" + p.slug),
    ...september10BlogPosts.map((p) => "/blog/" + p.slug),
    ...
    ...alternatives.map((p) => `/alternatives/${p.slug}`),
  ];
  const xml = `<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${urls.map((x) => `<url><loc>${base}${x}</loc></url>`).join("")}</urlset>`;
