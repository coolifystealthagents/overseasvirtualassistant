...
const PAGE_SIZE = 20;
const campaignPosts = [
  ...september24BlogPosts.map((p) => [p.slug, { title: p.title, description: p.description }] as const),
  ...
  ...september4BlogPosts.map(
    (p) => [p.slug, { title: p.title, description: p.description }] as const,
  ),
  ...Object.entries(september3Meta),
  ...Object.entries(aug23Meta),
].map(([slug, p]) => ({
  slug,
  title: p.title,
